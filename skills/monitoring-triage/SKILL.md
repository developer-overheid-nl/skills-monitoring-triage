...
| `github.com/logius-standaarden/<std>` commit `Beheer: verander specStatus naar 'DEF'` + `logius-standaarden.github.io/<std>` draft rebuild + `publicatie` commit `Release: <std>` (all on one day) | any ls-* | **A DEF publication, not beheer.** Logius flips `specStatus` to DEF on master, the publish workflow writes `<pubDomain>/<shortName>/<version>/` into `publicatie`, then develop goes back to WV (so the draft showing WV again means nothing). Check `curl -sL https://gitdocumentatie.logius.nl/publicatie/<pubDomain>/<shortName>/ \| grep -oE '"specStatus": *"[A-Z]+"'`. If it says DEF and the skill still says CV/WV, update the version table in SKILL.md and conflicts.md (PR), then close with the PR reference. |
| Issue body says `Repository is gearchiveerd` (type `github_repo`, often without a visible archive commit) | any | **Not a false positive.** Verify with `gh api repos/OWNER/REPO --jq .archived`. Mark the repo inline in the SKILL.md repo table and conflicts.md (`— **gearchiveerd** <maand jaar>`, no emoji, no separate table; see ls-egov, ls-api), PR, then close with the PR reference. |
